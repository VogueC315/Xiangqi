...
class Piece:
    ...
    def __init__(self, couleur, ligne, colonne):
        self.couleur = couleur      # Définit l'appartenance d'une pièce à un joueur
    ...
    def deplacer(self, nouvelle_ligne, nouvelle_colonne):
        ...
        self.ligne = nouvelle_ligne         # Mise à jour du numéro de ligne
        self.colonne = nouvelle_colonne     # Mise à jour du numéro de colonne
...
class General(Piece):
    ...
    def mouvements_possibles(self,jeu):
        mouvements=[[self.ligne+1,self.colonne],[self.ligne-1,self.colonne],[self.ligne,self.colonne+1],[self.ligne,self.colonne-1]]
        mouv_poss=[]
        for mouv in mouvements:
            if self.mouvement_valide(mouv[0], mouv[1], self):
                # Vérifie si une collision a lieu
                if self.chemin_libre(mouv[0], mouv[1], self):
                    mouv_poss.append(mouv)
        return mouv_poss
...
class Conseiller(Piece):
    ...
    def mouvement_valide(self, nouvelle_ligne, nouvelle_colonne, plateau):
        ...
        if self.couleur == 'white':
            # Conseillers blancs restreint aux colonnes 4 à 6 et lignes 1 à 3
            if not self.zone_autorisee(nouvelle_ligne, nouvelle_colonne, colonnes_autorisees=range(3, 6), lignes_autorisees=range(0, 3)):
                return False
        else:
            # Conseillers noirs restreint aux colonnes 4 à 6 et lignes 8 à 10
            if not self.zone_autorisee(nouvelle_ligne, nouvelle_colonne, colonnes_autorisees=range(3, 6), lignes_autorisees=range(7, 10)):
                return False
    ...
    def mouvements_possibles(self,jeu):
        mouvements=[[self.ligne+1,self.colonne+1],[self.ligne-1,self.colonne-1],[self.ligne-1,self.colonne+1],[self.ligne+1,self.colonne-1]]
        mouv_poss=[]
        for mouv in mouvements:
            if self.mouvement_valide(mouv[0], mouv[1], self):
                # Vérifie si une collision a lieu
...
class Elephant(Piece):
    ...
    def mouvement_valide(self, nouvelle_ligne, nouvelle_colonne, plateau):
        ...
        if self.couleur == 'white':
            # Elephants blancs restreint aux lignes 1 à 5
            if not self.zone_autorisee(nouvelle_ligne, nouvelle_colonne, lignes_autorisees=range(0, 5)):
                return False
        else:
            # Elephants noirs restreint aux lignes 6 à 10
    ...
    def mouvements_possibles(self,jeu):
        mouvements=[[self.ligne+2,self.colonne+2],[self.ligne-2,self.colonne-2],[self.ligne-2,self.colonne+2],[self.ligne+2,self.colonne-2]]
        mouv_poss=[]
        for mouv in mouvements:
            if self.mouvement_valide(mouv[0], mouv[1], self):
                # Vérifie si une collision a lieu
...
class Cavalier(Piece):
    ...
    def mouvements_possibles(self,jeu):
        mouvements=[[self.ligne+2,self.colonne+1],[self.ligne+2,self.colonne-1],[self.ligne-2,self.colonne+1],[self.ligne-2,self.colonne-1],[self.ligne+1,self.colonne+2],[self.ligne+1,self.colonne-2],[self.ligne-1,self.colonne+2],[self.ligne-1,self.colonne-2]]
        mouv_poss=[]
        for mouv in mouvements:
            if self.mouvement_valide(mouv[0], mouv[1], self):
                # Vérifie si une collision a lieu
...
class Char(Piece):
    ...
    def mouvements_possibles(self,jeu):
        mouvements=[[i,self.colonne] for i in range(10)]+[[self.ligne,i] for i in range(9)]
        mouv_poss=[]
        for mouv in mouvements:
            if self.mouvement_valide(mouv[0], mouv[1], self):
                # Vérifie si une collision a lieu
...
class Soldat(Piece):
    ...
    def mouvements_possibles(self,jeu):
        ...
        if self.couleur == 'white':
            if self.ligne <= 4:
                mouvements=[[self.ligne + 1,self.colonne]]
            else :
                mouvements=[[self.ligne + 1,self.colonne],[self.ligne ,self.colonne+1],[self.ligne ,self.colonne-1]]
        elif self.couleur == 'black' :
            if self.ligne >= 5:
                mouvements=[[self.ligne - 1,self.colonne]]
            else :
                    ...
                    if self.piece_selectionnee.mouvement_valide(ligne, colonne, self):
                        ...
                        if self.piece_selectionnee.chemin_libre(ligne, colonne, self):
                            ...
                            if piece_capturee:
                                ...
                                self.matrice[piece_capturee.ligne][piece_capturee.colonne] -= COULEUR_PIECE_J1
...
# Coder fonction rivière et fonction get_mouvements
